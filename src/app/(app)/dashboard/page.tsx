    ...
    <div id="dashboard-content" className="max-w-6xl mx-auto p-4 md:p-6 space-y-5">
      ...
      <div className="flex overflow-x-auto snap-x snap-mandatory gap-3 pb-1 md:grid md:grid-cols-4 md:overflow-visible md:pb-0">
        {[
          ...
          ...(prospects.length > 0 ? [{ icon: <Target className="size-4 text-[#a78bfa]" />, iconBg: "bg-[#a78bfa]/12", label: "Leads", value: `${fmt(Math.round(weightedPipeline))}\u20AC`, sub: `${prospects.length} prospect${prospects.length > 1 ? "s" : ""} en cours`, tip: "CA pondéré par probabilité de conversion de tes prospects" }] : []),
        ].map((kpi) => (
